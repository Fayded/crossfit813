...
add_action('admin_enqueue_scripts', 'rocknrolla_admin_scripts');
	...
	if (!is_admin() ){
		function rocknrolla_front_scripts(){		
			...
			wp_register_script('rnrVideoPlayer', RNR_INDEX_JS .'/jquery.mb.YTPlayer.js', true);	
			...
			if($smof_data['rnr_home_type']=="Video") { 
			   wp_enqueue_script('rnrVideoPlayer');
			}
...
function rocknrolla_print_styles() {  	
	if ( !is_admin() ){
		...
		wp_register_style( 'rnrYTPlayer', RNR_INDEX_CSS. '/YTPlayer.css', array(), '1', 'all' );
